...
STROKES WAG(STROKES S0, STROKES S1, float t){
  SHAPES Si0 = S0.convertShape();
  SHAPES Si1 = S1.convertShape();
  SHAPES St = new SHAPES();
  for(int i=0;i<Si0.lv;i++){
    if(i<=Si0.lv-2){
  ...
  St.lv = Si0.lv;
  St.av = Si0.av;
  return St.convertStroke(S0.G[0]);
}

STROKES WAGLINEAR(STROKES S0, STROKES S1, float t){
  SHAPES Si0 = S0.convertShape();
  SHAPES Si1 = S1.convertShape();
  SHAPES St = new SHAPES();
  for(int i=0;i<Si0.lv;i++){
