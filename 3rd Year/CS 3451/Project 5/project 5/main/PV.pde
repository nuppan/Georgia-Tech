...
pt P(pt O, float x, vec I, float y, vec J, float z, vec K) {return P(O.x+x*I.x+y*J.x+z*K.x,O.y+x*I.y+y*J.y+z*K.y,O.z+x*I.z+y*J.z+z*K.z);}  // O+xI+yJ+zK
...
float det(vec U, vec V) {return d(U,R(V)); }
...
void show(pt P, float s, vec I, vec J, vec K) { stroke(red); show(P,s,I); stroke(green); show(P,s,J); stroke(blue); show(P,s,K); stroke(black); }; // render sphere of radius r and center P
