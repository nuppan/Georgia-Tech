...
// ===== point class
class pt { float x=0,y=0,z=0; int radius=1,interval=1; vec velocity=new vec(); ;int mass=1;
   void setRadius(int r){radius=r;}
   void setInterval(int i){interval=i;}
   void setV(vec V){velocity.x=V.x;velocity.y=V.y;velocity.z=V.z;}
   pt () {}; 
   ...
   pt randomParticle(int r){
     float angle = random(0, 360);
     float a = x + (radius+r)*cos(angle);
     float b = y + (radius+r)*sin(angle);
     float c = z + (radius+r)*cos(angle); 
     pt newpt = new pt(a,b,c);
     newpt.setRadius(r);
     return newpt;
   }
