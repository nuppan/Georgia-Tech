...
void draw() {  
  ...
  if(interval>=particleGenerator.interval && numParticles<=50){
 ...
 float[] computeS(float T){
   ...
   for(int i=0;i<numParticles-1;i++){
       ...
       for(int j=i+1;j<numParticles;j++){ //Compare all pairs of particles
         ...
         vec V = B.velocity;
         vec UV = M(V, U);
         float Acoeff = d(UV, UV);
         vec AB = V(A, B);
         float Bcoeff = d(V(2, AB), UV);
         float Ccoeff = d(V(A, B), V(A,B)) - pow(A.radius+B.radius, 2);
         float discrim = pow(Bcoeff, 2) - 4*Acoeff*Ccoeff;
         if (discrim >= 0){
           float posSol = ((-1)*Bcoeff + pow(discrim, .5))/(2*Acoeff);
           float minusSol = ((-1)*Bcoeff - pow(discrim, .5))/(2*Acoeff);
           //println("posSol: " + posSol + " minusSol: " + minusSol);
           if (posSol > 0 && posSol < T){
               if (result[2] > posSol){
                  found = true;
                  result[0] = i;
                  result[1] = j;
           ...
           if (minusSol > 0 && minusSol < T){
             if (result[2] > minusSol){
                found = true;
                result[0] = i;
                result[1] = j;
                result[2] = minusSol; 
...
void keyReleased() {
   ...
   if(key=='r') {if(particleGenerator.radius<=5){particleGenerator.radius=0;}else{particleGenerator.radius+=5;}}
   if(key=='R') {particleGenerator.radius+=5;}
   if(key=='l') {if(particleGenerator.interval<=0){particleGenerator.interval=0;}else{particleGenerator.interval-=5;}}
   if(key=='L') {particleGenerator.interval+=1;}
   U.set(M(J)); // reset camera up vector
   } 
